...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        while (mod_count < mbi->mods_count) {
            if (mod_count == 0)
            {
                filesystem_start = (uint32_t*) mod->mod_start ;
                filesystem_end = (uint32_t*) mod->mod_end; 
            }
    ...
    /*Initialize paging*/
    init_filesystem((uint8_t*)filesystem_start,(uint8_t*)filesystem_end) ;
